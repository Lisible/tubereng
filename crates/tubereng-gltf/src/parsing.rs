...
#[derive(Debug)]
pub struct Glb {
    pub(crate) _header: GlbHeader,
    pub(crate) gltf: GltfJson,
...
impl GlbParser {
        ...
        R: Read,
    {
        ...
        let binary_chunk_length =
            read_next_u32(&mut read).map_err(GltfError::GlbChunkParseError)?;
        let binary_magic_number =
            read_next_u32(&mut read).map_err(GltfError::GlbChunkParseError)?;
        if binary_magic_number != Self::CHUNK_TYPE_BIN {
            return Err(GltfError::GlbInvalidMagicNumber);
        }

        let mut binary_data = vec![0u8; binary_chunk_length as usize];
        read.read_exact(&mut binary_data)
    ...
    fn parse_gltf_json_chunk<R>(mut read: &mut R) -> Result<GltfJson, GltfError>
    where
        R: Read,
    {
...
mod tests {
    ...
    fn simple_gltf() {
        ...
        let gltf: GltfJson = serde_json::from_str(gltf_str).unwrap();
